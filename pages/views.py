...
def dinner(request):
    ...
    # dinner = random.sample(menu, 2)  # 여러개 가져올 때 sample
    random_dinner = random.choice(menu)  # 한개만 가져올 때 choice

    return render(request, 'pages/dinner.html', {'dinner': random_dinner})


def greetingIU(request):
    return render(request, 'pages/greetingIU.html')
...
# throw.html 에서 보낸 값이 catch(request)에 저장된다.
def catch(request):
...
def artiiResult(request):
    ...
    return render(request, 'pages/artiiResult.html', context)
